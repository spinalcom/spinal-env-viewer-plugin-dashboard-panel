<template>
	<div
		...
		:style="style"
	>
		<div v-if="endpoint" class="endpointDiv">
			<div class="name" v-tooltip="endpoint.name">
				{{ endpoint.name }}
			</div>
			<div
				class="value"
			...
			<div class="btnGroup">
				...
				>
					<md-icon class="endpointIcons" :class="{ alarmBtn: isInAlarm }">
						error_outline
					</md-icon>
				</md-button>
				<!-- Alarm Icon -->
	...
	export default {
		...
		methods: {
			selectEndpoint: function () {
				this.$emit("selectEndpoint", this.endpointModel.id.get());
			},
